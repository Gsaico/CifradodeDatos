...
if (!MS.Support.Fms.Survey) {
    ...
    MS.Support.Fms.SurveyQuestionOperator =
	{
	    ...
	    emptyOperator: function (question) {
	        ...
	        this.populateVariablesRandom = function () {
	            return [];
	        }
	    ...
	    choiceOperator: function (question) {
	        ...
	        function getOptionTextRandom(index) {
	            var e = question.elements[index];
	            return (e.visible && e.checked) ? ((index == lastRadioIndex && hasOptionalTextBox) ? e.parentNode.nextSibling.innerHTML : e.parentNode.nextSibling.innerHTML) : null;
	        }
	        ...
	        function getOptionVariableRandom(index) {
	            var e = question.elements[index];
	            var variable = e.getAttribute("id");
	            if (variable != null && variable != "") {
	                return { name: variable, value: getOptionTextRandom(index) };
	            }
	            return null;
	        }
	        ...
	        this.populateVariablesRandom = function () {
	            ...
	            for (var index = 0; index <= lastRadioIndex; ++index) {
	                var optionVariable = getOptionVariableRandom(index);
	    ...
	    ghrOperator: function (question) {
	        ...
	        this.populateVariablesRandom = function () {
	            ...
	            var optionVariable = getOptionVariableRandom();
	        ...
	        function getOptionVariableRandom() {
	            var variableList = new Array();

	            for (var row = 0; row < ghrRows.length; ++row) {
	                var divs = ghrRows[row].getElementsByTagName("DIV");
	                for (i = 0; i < divs.length; i++) {
	                    if (divs[i].className == "GHR_QUESTIONTEXT") {
	                        variableList[row] = divs[i].innerText;
	                    }
	                }
	            }
    ...
    MS.Support.Fms.SurveyQuestion = function (domObject, page) {
        ...
        this.populateVariablesRandom = function () {
            return operator.populateVariablesRandom();
        }
    ...
    MS.Support.Fms.Survey = function (id, config) {
        ...
        //This function fills the suspended responses in the survey rendered
        this.PreFillResponses = function () {
            var xml = this.getResumeSurveyParams();

            //Clear resume survey params from submit fields
            this.submitFields["RESUMESURVEYPARAMS"] = null;
            ...
            //Iterate through each question tag
            $Questions = $xml.find("Question");
            $.each($Questions,
                function (x, y) {
                    //Obtain the question attributes
                    $Question = $(y);
                    $QuestionID = $Question.attr("QuestionID");
                    $QuestionType = $("#SURVEYQUESTION_" + $QuestionID + " input[name='type']").attr('value');
                    $Answers = $Question.find("Answer");

                    //Iterate through answer tag(s)
                    $.each($Answers,
                    function (i, j) {
                        //Obtain the answer attributes
                        $Answer = $(j);
                        $AnswerId = $Answer.attr("AnswerId");
                        $AnswerValue = $Answer.attr("AnswerValue");
                        $AnswerText = $Answer.attr("AnswerText");

                        //Fill the answer based on the question type
                        switch ($QuestionType) {
                            case "CHECK-LIST":
                                $("input[type='checkbox'][name*='"+$QuestionID+"'][id='" + $AnswerId + "']").attr('checked', 'checked');
                                break;

                            case "CHOICE":
                                $("input[type='radio'][name*='"+$QuestionID+"'][id='" + $AnswerId + "']").attr('checked', 'checked');
                                break;

                            case "CHOICE-SEQ": case "CHOICE-HOR":
                                $("input[type='radio'][name*='"+$QuestionID+"'][id='" + $AnswerId + "'][value='" + $AnswerValue + "']").attr('checked', 'checked');
                                break;

                            case "CHOICE-LIST": case "MULTI-OPTION":
                                $("#" + $AnswerId).prop('selected', 'selected');
                                break;

                            case "TEXT-LINE":
                                $("input[type='text'][name='TEXT_" + $QuestionID + "']").attr('value', $AnswerText);
                                break;

                            case "TEXT-BLOCK":
                                $("textarea[name='TEXTBLOCK_" + $QuestionID + "']").val($AnswerText);
                                break;

                            case "TEXT-NUMERIC":
                                $("input[type='number'][name='NUMERIC_" + $QuestionID + "']").val($AnswerValue);
                                break;

                            case "TEXT-DATE":
                                $("input[name='DATE_" + $QuestionID + "']").val($AnswerValue);
                                break;
                        ...
                        $("input[type='text'][id='" + $AnswerId + "']").removeAttr('disabled');
                        $("input[type='text'][id='" + $AnswerId + "']").attr('value', $AnswerText);
                    });
                }
            );

            //Now, Navigate to the suspended page

            //Get all the sectionIds from the xml
            ...
            $.each($Sections,
               function (x, y) {                   
                   $Section = $(y);
                   sectionIds.push($Section.attr("SectionID"));
               }
            );                     

            //Get the pageIds from the rendered survey
            var suspendedPageId;
            var pageToNavigate = this.pages.length - 1;
            while (pageToNavigate >= 0) {
                if (jQuery.inArray(this.pages[pageToNavigate].getPageId(), sectionIds) >= 0) {
                    suspendedPageId = this.pages[pageToNavigate].getPageId();
                    break;
                }
                pageToNavigate--;
            }
            ...
            //Navigate to page
            while (pageToNavigate > 0 && this.pages[currentPageIndex].getPageId()!=suspendedPageId) {
                this.next();
                pageToNavigate--;
            }
        }
                ...
                if (currentPageIndex == -1) {
                    ...
                    if (validateResult.isValid) {
                        ...
                        if (currentPageIndex >= this.pages.length || this.thankyou != null && (this.pages[currentPageIndex] == this.thankyou)) {
                            ...
                            page.show();
                            var questions = this.pages[currentPageIndex - 1].questions;
                            var qArrayLength;
                            var subVariables = '';
                            var qspan = '';
                            var count = 0;
                            var spanvar = '';
                            for (var index1 = 0; index1 < questions.length; ++index1) {
                                if (questions[index1].getQuestionType() == 'CHECK-LIST' || questions[index1].getQuestionType() == 'CHOICE-SEQ') {
                                    spanvar = new Array();
                                    qArrayLength = questions[index1].domObject.getElementsByTagName('label').length;
                                    if (qArrayLength > 0) {
                                        for (n = 0; n < qArrayLength; n++) {
                                            qspan = questions[index1].domObject.getElementsByTagName('label')[n].getAttribute('spanvalue')
                                            if (qspan != null || qspan != undefined) {
                                                var subVariables = questions[index1].populateVariablesRandom();
                                                var multiq = qspan.split(",");
                                                for (m = 0; m < multiq.length; m++) {
                                                    spanvar.push(multiq[m])
                                                }
                                            }
                                        }
                                        if (spanvar.length > 0) {
                                            dynamicArray = new Array();
                                            dynamicArray = subVariables;
                                            this.randomPiping(dynamicArray, spanvar);
                                        }
                                    }
                                }
                            }
        ...
        this.suspend = function () {
            if (confirm("You have suspended the survey. To resume, please click on the original link you used to start the survey. You can now close this window.")) {
                this.pages[currentPageIndex].modified = true;
                this.submit("suspend");                
            }
        }
        ...
        this.getResumeSurveyParams = function () {
            return this.submitFields["RESUMESURVEYPARAMS"];
        }
        ...
        this.randomPiping = function (dynamicArray, spanvar) {
            myArray = [];
            ...
            var spanval = document.getElementById(spanvar[0]).innerText;
            if (spanval != "") {
                for (s = 0; s < myNewArray.length; s++) {
                    if (myNewArray[s] == spanval)
                        return;
                }
            }
            for (s = 0; s < myNewArray.length; s++) {
                if (myNewArray[s] != null) {
                    if (myNewArray[s] != '') {
                        bool = true;
                        break;
                    }
            ...
            if (spanval != '' && bool == false) {
                for (s = 0; s < spanvar.length; s++) {
                    document.getElementById(spanvar[s]).innerText = '';
                }
            }
            ...
            if (bool == true) {
                while (strvalue == null) {
                    if (rand != -1) {
                        rand = Math.floor(Math.random() * (myNewArray.length));
                        var strvalue = myNewArray[rand];
                        if (strvalue != "undefined" || strvalue != '') {
                            if (strvalue != null) {
                                for (l = 0; l < spanvar.length; l++) {
                                    var span = document.getElementById(spanvar[l]);
                                    if (span != null) {
                                        if (span != '') {
                                            while (span.firstChild) { span.removeChild(span.firstChild); }
                                            span.appendChild(document.createTextNode(strvalue));
                                            span.innerHtml = strvalue;
                                            if (l == spanvar.length - 1) {
                                                rand = -1; break;
                                            }
                                        }
                                    }
